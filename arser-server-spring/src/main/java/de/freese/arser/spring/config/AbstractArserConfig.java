 ...
 * @since 22.08.26
 */
public abstract class AbstractArserConfig {
    @Bean
    ...
    Repository localSnapshots(@Value("${arser.workingDir}") final Path workingDir) {
        ...
        return FileRepository.of(config);
    }
    ...
    protected HttpRepositoryConfig createGradleLibsReleasesConfig(final Path workingDir) {
        return HttpRepositoryConfig.builder()
    ...
    protected HttpRepositoryConfig createMavenCentralConfig(final Path workingDir) {
        return HttpRepositoryConfig.builder()
                .name("maven-central")
